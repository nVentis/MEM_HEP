...
def build_param_net(hidden_channels:int, num_bins:int, n_layers:int=5,
                    activation:Callable=torch.nn.ReLU, dropout_probability:float=0.,
                    bay_nsf:bool=True):
    ...
    def builder(in_features , out_features, options:None):
        ...
        nn.init.constant_(seq.final_layer.weight, 0.01)
        nn.init.constant_(seq.final_layer.bias, 0.01)
...
class NeuralImportanceSamplingIntegrator(ImportanceSamplingIntegrator):
    def __init__(self,
        ...
        for mask in masks:
            ...
            flows += [PiecewiseRationalQuadraticSpline(mask, build_param_net(hidden_channels=hidden_channels, num_bins=num_bins, n_layers=hidden_layers,), num_bins=num_bins )]
    ...
    def step(self, loss_func:Optional[Callable]=kl_div, n_samples:int=64000, with_loss:bool=True):            
    ...
    def adapt(self, loss_func:Callable=exp_div, n_samples:int=64000,
        ...
        for epoch in (pbar := tqdm(range(n_epochs_max), disable=self.silent)):
            ...
            accept_epoch = (self.last_n_worse <= n_max_fail)
    ...
    def evaluate(self, use_adaption:bool=True, n_samples:Optional[int]=None, ):
        if n_samples is not None:
            with torch.no_grad():
                mean, var, sigma = self.step(n_samples=n_samples, with_loss=False)
